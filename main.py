...
def get_json() -> Dict:
    ...
    parser = argparse.ArgumentParser(
        description="Generate python classes directly from JSON"
    )
    group = parser.add_mutually_exclusive_group(required=True)
    group.add_argument(
        "-s", type=str, help="Provide the JSON directly via the command line"
    )
...
def extract_classes_from_json(p_json: Dict, keys_props: Dict) -> Dict:
    ...
    for key in p_json.keys():
        if (
            isinstance(p_json[key], list)
            and len(p_json[key]) >= 1
            and isinstance(p_json[key][0], dict)
        ):
...
def build_class(key: str, p_dict: Dict) -> None:
    ...
    with open(filename, "w") as pyfile:
        ...
        for prop in p_dict.keys():
            converted = convert(prop)
            pyfile.write("        self.{} = kwargs.get('{}', None)".format(converted, prop))
            pyfile.write("\n")
